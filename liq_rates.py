...
def processing_ruonia(date_from, date_to):
    def get_ruonia_rates(dt_min, stop_date):
        link = f'https://cbr.ru/hd_base/ruonia/dynamics/?UniDbQuery.Posted=True&UniDbQuery.From={dt_min}&UniDbQuery.To={stop_date}'
    ...
    ruonia['Значение, %'] = ruonia['Значение, %'].apply(lambda x: float(x.replace(",", ".")))
...
def convert_type_column(df):
    ...
    for column in list(df.columns)[1:]:
        df[column] = df[column].apply(lambda x: float(str(x.replace(',', '.').replace(' ', ''))) if isinstance(x, str) and (x != '--' and x != '—') else x)
    return df
...
def get_rates(df):
    ...
    rates_1y1y = [round(100 * ((1 + rates_2y[i])**2 / (1 + rates_1y[i]) - 1), 2) for i in range(len(rates_2y))]
    rates_3m3m = [round(100 * ((1 + rates_6m[i])**2 / (1 + rates_3m[i]) - 1), 2) for i in range(len(rates_6m))]
    rates_1m1m = [round(100 * ((1 + rates_2m[i])**2 / (1 + rates_1m[i]) - 1), 2) for i in range(len(rates_2m))]
    rates_6m6m = [round(100 * ((1 + rates_1y[i])**2 / (1 + rates_6m[i]) - 1), 2) for i in range(len(rates_1y))]
    return rates_1y1y, rates_3m3m, rates_1m1m, rates_6m6m


def get_spread_rates(key_rates, rates_1m1m, 
                     rates_3m3m, rates_1y1y, 
                     rates_6m6m, df):
...
def add_new_list(name_file, date_from, date_to):
    ...
    df_merge = get_spread_rates(key_rates, rates_1m1m, 
                                rates_3m3m, rates_1y1y, 
                                rates_6m6m, df_merge)
    with pd.ExcelWriter(name_file, 
                        engine='openpyxl', 
                        mode='a', 
...
def processing_dates(dates):
    ...
    for day in dates:
        ...
        try:
            ...
            title = [title if 'по ключевой ставке' in title else None for title in titles][0]
            meeting_date = day.find('div', class_='date col-md-5').\
                text.strip().replace('\xa0', ' ').replace(' года', '')
